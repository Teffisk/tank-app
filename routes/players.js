...
router.get("/results/", function(req, res) {
  ...
  request(urlAccount, function(error, response, body) {
    if (JSON.parse(body).meta.count == 1) {
      ...
      res.render("players/error", { error: error });
    }
  });
});
  ...
  request(urlRecent, function(error, response, body) {
    ...
    db.tank
      ...
      .then(function(tanks) {
        ...
        res.render("players/recent-vehicles", {
          recentTanks: sortedTanks,
          dbTanks: obj
        });
      });
  });
      ...
      .then(function(tanks) {
        ...
        recentTanks.forEach(function(rT) {
          ...
          if (recentObj[rT.tank_id]) {
            ...
            if (
              countPlayed === recentObj[rT.tank_id].next_tanks.length ||
              recentObj[rT.tank_id].next_tanks.length === 0
            ) {
            } else {
              grindTanks.push(rT);
            }
      ...
      .then(function() {
        db.tank
          ...
          .then(function(tanks) {
            ...
            res.render("players/current-grind", {
              recentTanks: grindTanks,
              timestamp: timestamp,
              dbTanks: grindObj
            });
          });
